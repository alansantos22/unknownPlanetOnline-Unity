...
namespace GoPath.Navigation
{
    public class GridManager : MonoBehaviour
    {
        ...
        private void InitializeGrid()
        {
            ...
            for (float x = mapBounds.min.x; x <= mapBounds.max.x; x += gridSpacing) // Modificado
            {
                for (float y = mapBounds.min.y; y <= mapBounds.max.y; y += gridSpacing) // Modificado
                {
        ...
        private bool IsStrictlyWalkable(Vector2 position)
        {
            ...
            float checkRadius = gridSpacing * 0.25f; // Reduzido de 0.5f para 0.25f
        ...
        public List<Vector2> FindPath(Vector2 start, Vector2 end)
        {
            ...
            var openSet = new PriorityQueue<NavigationNode>();
        ...
        private bool IsPathSafe(Vector2 start, Vector2 end)
        {
            ...
            int checks = Mathf.Max(5, Mathf.CeilToInt(distance / (gridSpacing * 0.2f))); // Reduzido número de checks
